...
export function StageColumn({ stage, boardId, onOpenCard }: StageColumnProps) {
  ...
  const handleAddCard = (e: FormEvent<HTMLFormElement> | KeyboardEvent<HTMLTextAreaElement>) => {
    e.preventDefault()
    if (!newCardTitle.trim()) return
    const title = newCardTitle.trim()
    addCard(boardId, stage.id, title)
    log(ACTIVITY_TYPES.CARD_CREATED, `creó la tarjeta "${title}" en "${stage.name}"`)
    setNewCardTitle('')
    setIsAdding(false)
  }

  const handleRename = (e: FormEvent<HTMLFormElement> | React.FocusEvent<HTMLInputElement>) => {
    e.preventDefault()
    if (editName.trim() && editName.trim() !== stage.name) {
      const oldName = stage.name
      updateStage(boardId, stage.id, { name: editName.trim() })
      log(ACTIVITY_TYPES.STAGE_RENAMED, `renombró la etapa "${oldName}" a "${editName.trim()}"`)
    }
    setIsEditing(false)
  }
      ...
      <div className="flex items-center justify-between px-3 py-2.5">
        {isEditing ? (
          ...
          <h3 className="text-sm font-semibold text-surface-700">
            {stage.name}
            <span className="ml-2 text-xs font-normal text-surface-400">{visibleCards.length}</span>
          </h3>
        )}
        <DropdownMenu
          trigger={
        ...
        >
          <DropdownItem onClick={() => { setIsEditing(true); setEditName(stage.name) }}>
            <span className="flex items-center gap-2"><Pencil size={18} weight="duotone" /> Renombrar</span>
          </DropdownItem>
          <DropdownItem danger onClick={() => {
            ...
            <span className="flex items-center gap-2"><Trash size={18} weight="duotone" /> Eliminar</span>
          </DropdownItem>
        </DropdownMenu>
      </div>
        ...
        {isAdding ? (
          <form onSubmit={handleAddCard} className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <Button size="sm" type="submit">Añadir</Button>
              <Button size="sm" variant="ghost" type="button" onClick={() => setIsAdding(false)}>Cancelar</Button>
            </div>
          </form>
        ) : (
          <button
