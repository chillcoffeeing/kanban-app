...
export function InvitationsPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const res = await ApiClient.get("/invitations/pending");
        ...
        if ((e as any)?.name !== "AbortError") {
          handleError(e, "Error al cargar invitaciones");
        }
      } finally {
        setLoading(false);
      }
  ...
  const accept = async (invitation: PendingInvitation) => {
    ...
    try {
      ...
      if ((e as any)?.name !== "AbortError") {
        handleError(e, "Error al aceptar la invitación");
      }
    } finally {
  ...
  const reject = async (invitation: PendingInvitation) => {
    ...
    try {
      ...
      setInvitations(
        invitations.filter(
          (invitationItem) => invitationItem.id !== invitation.id,
        ),
      );
    } catch (e: unknown) {
      if ((e as any)?.name !== "AbortError") {
        handleError(e, "Error al rechazar la invitación");
      }
    } finally {
      setActionInProgress(null);
    }
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-neutral-dark/60">
          Has sido invitado a {invitations.length} tablero
